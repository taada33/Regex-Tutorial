...
Flags are used in regular expressions to allow for additional specified functionality. Three flags(```i``` and ```g``` and ```m```) will be covered in this tutorial. For instance, following your regex with an ```i``` flag such as
